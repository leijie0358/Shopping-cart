			...
			switch($prefix)
			{
				...
				default :
				{
					...
					if(substr(trim($lstr),-1) == ";")
					{
						...
						//更换表前缀
						$sqlStr   = str_ireplace('{pre}',$db_pre,join($sqlArray),$rcount); 
						
						// showProgress(array('isError' => true,'message' => $lstr."bcd"));
						$sqlArray = array();
...
{
	showProgress(array('isError' => false,'message' => '开始安装数据库'));
	global $db_pre;
	...
	showProgress(array('isError' => false,'message' => '数据库服务器连接成功'));
	
	//检测SQL安装文件
	$sql_file = ROOT_PATH.'./install/database.sql';
	if(!file_exists($sql_file))
	{
		showProgress(array('isError' => true,'message' => '安装的SQL文件'.basename($sql_file).'不存在'));
	}
	showProgress(array('isError' => false,'message' => '检测到数据库脚本文件'));
	
	//检测测试数据SQL文件
	$sql_test_file = ROOT_PATH.'./install/database_test.sql';
	...
	{
		showProgress(array('isError' => false,'message' => '没有找到数据库,开始创建'));
	...
	showProgress(array('isError' => false,'message' => '数据库连接成功,开始分析脚本')); 
	...
	$adminSql = 
	"INSERT INTO `user` (`id`, `role_id`, `username`, `password`, `realname`, `email`, `workphone`, `mobilephone`, `company`, `department`, `workaddress`, `work_addr_province`, `work_addr_city`, `work_addr_area`, `work_addr_street`, `work_addr_detail`, `controller`, `action`, `banned`, `ban_reason`, `newpass`, `newpass_key`, `newpass_time`, `last_ip`, `last_login_time`, `create_time`, `modify_time`) VALUES
	(1, 1, '$admin_user', '".md5($admin_pwd)."', '管理员', 'admin@localhost.com', '', '18726021081', '马鞍山携程软件', '技术部', '马鞍山花山区翠岛华庭1-602', '安徽', '马鞍山', '', '', '花山区翠岛华庭1-602', 'welcome', 'index', 0, NULL, NULL, NULL, NULL, '127.0.0.1', '2011-04-05 04:56:38', '2011-04-05 04:56:32', '2011-04-05 04:56:38');";
