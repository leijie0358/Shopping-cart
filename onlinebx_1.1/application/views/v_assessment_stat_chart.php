...
<script type="text/javascript"> 
	...
	Highcharts.visualize = function(table, options) {
		...
		$('tr', table).each( function(i) {
			var tr = this;
			var len = $('th, td', tr).length;
			var index  = 0;
			$('th, td', tr).each( function(j) {
				if (j > 0 && j%2==1 && j != (len-1)) { // skip first column
	...
	$(document).ready(function() {			
		...
		{
			text = $('#bx_time_end').val() + "之前的客户满意度统计";
		}
		else if($('#bx_time_end').val()=="")
		{
			text = $('#bx_time_begin').val() + "之后的客户满意度统计";
		}
		else
		{
			text = $('#bx_time_begin').val() + "到" + $('#bx_time_end').val() + "的客户满意度统计";
		}
